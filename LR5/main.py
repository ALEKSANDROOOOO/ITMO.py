...
def main() -> None:
    ...
    #output_bin_tree(gen_bin_tree()) # для красивого вывода - заменить в gen_bin_tree return (закомментирован)
    #print(gen_bin_tree())
    

def gen_bin_tree(height: int=6, root: int=15, l_b: int=lambda x: 2*(x+1), r_b: int=lambda x: 2*(x-1)) -> list[int]:
...
def output_bin_tree(tree: list[list[int]], spaces: int = 128) -> None:
